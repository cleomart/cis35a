...
public class VerifySudokuSolution {
	...
	private static boolean checkValues(int[][] grid, int min, int max)
	{
		for (int i = 0; i < grid.length; i++)
			for (int j = 0; j < grid.length ; j++)
				...
				{
					// found any values greater than max or less than min, so return false
					return false; 
				}
		
		// didn't find any values greater than max or less than min, so return true
		return true;
	}
	...
	private static boolean checkColumn(int[][] grid, int whichCol)
	{
		...
		for (int row = 0; row < size; ++row)
		{
			// set found[x - 1] to be true if we find x in the column
			int index = grid[row][whichCol] - 1;
			if (!found[index])
				found[index] = true;
			else
				return false; // found it twice, so return false
		}
		// didn't find any number twice, so return true
		return true;
	}
	...
	private static boolean checkSquare(int[][] grid, int baseRow, int baseCol, int subSquareSize)
	{
		...
		for (int i = 0; i < subSquareSize; i++)
			...
			{
				// set found[x - 1] to be true if we find x in the column
				int index = grid[baseRow+i][baseCol+j] -1;
				if (!found[index])
					found[index] = true;
				else
					return false; // found it twice, so return false
